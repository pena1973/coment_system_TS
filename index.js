...
function initBlock() {
    ...
    // коменты
    // Первые    
    let firstComentsLength = localStorage.getItem('firstComents-length');
    firstComentsLength = (firstComentsLength == null) ? 0 : Number(firstComentsLength);
    for (let indF = 0; indF < firstComentsLength; indF++) {
        let coment_ = localStorage.getItem("firstComent" + indF);
        if (coment_ == null)
            continue;
        let jsonF = JSON.parse(coment_);
        let firstComent = new FirstComent(jsonF.userName, jsonF.msg, jsonF.foto, jsonF.rating, jsonF.favorite, jsonF.time, jsonF.id);
        firstComents.push(firstComent);
    }
    // подчиненные
    let secondComentsLength = localStorage.getItem('secondComents-length');
    secondComentsLength = (secondComentsLength == null) ? 0 : Number(secondComentsLength);
    for (let indS = 0; indS < secondComentsLength; indS++) {
        let coment__ = localStorage.getItem("secondComent" + indS);
        if (coment__ == null)
            continue;
...
function renderComents(onlyFavorites = false) {
    firstComents.forEach(firstComent => {
        if (onlyFavorites && !firstComent.getJson().favorite) {
            ...
            if (!parent)
                return;
            let firstComentDOM = firstComent.render(parent);
            ...
            secondComentsOfFirst.forEach(secondComent => {
                if (onlyFavorites && !secondComent.getJson().favorite) { }
                    ...
                    secondComent.render(firstComentDOM);
            });
        }
    });
}
// Удаление коментариев
function clearComents() {
...
function processSetAtributes(id, action) {
    let coment = firstComents.find(firstComent => +firstComent.getJson().id == id);
    if (!coment) {
        coment = secondComents.find(secondComent => +secondComent.getJson().id == id);
    }
    if (coment) {
        switch (action) {
            case 'favorites':
...
function addComent(msg, id = "") {
    ...
    // иначе это подчиненный комент (second)
        ...
        let firstComent = firstComents.find(firstComent => +firstComent.getJson().id == id);
        if (!firstComent)
            return; // ничего не добавляем  -  но это нереальная ситуация
        if (msg != "") {
            ...
            let firstComentDom = document.getElementById('fc' + id);
            if (!firstComentDom)
                return; // ничего не добавляем  -  но это нереальная ситуация при добавлении ответа исходныйкомент будет на сайте
